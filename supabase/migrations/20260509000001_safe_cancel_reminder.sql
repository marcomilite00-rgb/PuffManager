-- Migration: Cancel Reminder to Reservation (Safe version)
-- Purpose: Prevents cancelling orders that have already been archived in old load sessions.
...
BEGIN
    ...
    -- NEW GUARD-RAIL: Prevent cancelling historically archived orders
    IF v_order.is_archived = true THEN
        RAISE EXCEPTION 'Impossibile annullare: il pagamento parziale è già stato storicizzato nella chiusura di un carico precedente. Questa operazione distruggerebbe lo storico contabile.';
    END IF;
