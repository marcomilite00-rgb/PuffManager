-- Fix: UPDATE inventory without WHERE clause is blocked by Supabase safety checks.
-- Also ensures product_variants update covers ALL variants (not just active).
...
BEGIN
    ...
    -- Reset inventory quantities (WHERE clause required by Supabase safety)
    UPDATE inventory SET qty = 0
    WHERE variant_id IS NOT NULL;

    -- Apply new unit_cost to ALL variants (active AND inactive) so price is always current
