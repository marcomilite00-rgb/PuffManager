...
export const PaymentModal: React.FC<PaymentModalProps> = ({
    isOpen, onClose, onConfirm, totalAmount, initialAmount = ''
}) => {
    const inputRef = useRef<HTMLInputElement>(null);
    const [amount, setAmount] = React.useState(initialAmount);

    useEffect(() => {
        if (isOpen) { setAmount(initialAmount); setTimeout(() => { inputRef.current?.focus(); }, 50); }
    ...
    const handleConfirm = () => {
        ...
        if (isNaN(value) || value < 0) { alert('Inserisci un importo valido'); return; }
        onConfirm(value);
    };
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <motion.div initial={{ scale: 0.92, y: 20 }} animate={{ scale: 1, y: 0 }}
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-slate-400 mt-2 text-sm">Totale Dovuto: <span className="text-success font-bold tabular-nums">€{safeNumber(totalAmount).toFixed(2)}</span></p>
                </div>
                <div className="space-y-4">
                    ...
                    <div className="flex gap-3 mt-8">
                        <button onClick={onClose}
                            className="flex-1 py-4 bg-white/5 text-slate-400 font-bold rounded-xl hover:bg-white/10 transition-colors text-xs uppercase tracking-widest">
                            Annulla
                        </button>
                        <button onClick={handleConfirm}
                            className="flex-1 py-4 bg-gradient-to-r from-success to-success-dark text-surface-950 font-black rounded-xl hover:shadow-xl shadow-lg shadow-success/20 transition-all text-xs uppercase tracking-widest">
                            Incassa
                        </button>
                    </div>
                </div>
