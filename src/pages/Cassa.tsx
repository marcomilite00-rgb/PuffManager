...
export const Cassa: React.FC = () => {
    ...
    const [variantsMap, setVariantsMap] = useState<Record<string, number>>({}); // Map variant_id -> unit_cost
    ...
    useEffect(() => {
        ...
        const subscription = supabase
            ...
            .on('postgres_changes', { event: '*', schema: 'public', table: 'product_variants' }, () => fetchData()) // Also listen to cost changes
            .subscribe();
    ...
    const fetchData = async () => {
        const [ordersRes, staffRes, settingsRes, remindersRes, variantsRes] = await Promise.all([
            supabase
                .from('orders')
    ...
    const totals = useMemo(() => {
        ...
        // Net for THIS session = Gross - Reinvestment
        // (This matches the logic in perform_closing_load)
    ...
    const staffEarnings = useMemo(() => {
        ...
        sessionOrders.forEach(order => {
            if (earnings[order.sold_by_staff_id] !== undefined) {
                // 1. REVENUE (Lordo): Soldi effettivamente incassati per questo ordine
                const totalPaid = (order as any).payments?.reduce((acc: number, p: any) => acc + Number(p.amount), 0) || 0;

                // 2. GUADAGNO NETTO (Margine): Differenza tra Prezzo Vendita e Costo Unitario ATTUALE
                // Utilizza il costo dal menu sviluppatore (live) se disponibile, altrimenti fallback allo storico
                const items = (order as any).items || [];
                const totalProfit = items.reduce((acc: number, item: any) => {
                    const currentCost = variantsMap[item.variant_id] !== undefined
                        ? variantsMap[item.variant_id]
                        : Number(item.unit_cost || 0);

                    const margin = (Number(item.unit_price_final) - currentCost) * Number(item.qty);
                    return acc + margin;
                }, 0);

                earnings[order.sold_by_staff_id].gross += totalPaid;
                earnings[order.sold_by_staff_id].net += totalProfit;
            }
        });
        ...
        return staff.map(s => ({
            ...
            net: earnings[s.id].net,
            percent: totalGross > 0 ? (earnings[s.id].gross / totalGross) * 100 : 0
        })).sort((a, b) => b.gross - a.gross);
    }, [sessionOrders, staff, variantsMap]); // Depend on variantsMap
